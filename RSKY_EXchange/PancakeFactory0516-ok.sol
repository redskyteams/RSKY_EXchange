...
contract PancakePair is IPancakePair, PancakeERC20 {
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        ...
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    uint numerator = totalSupply.mul(rootK.sub(rootKLast)).mul(38);
                    uint denominator = rootK.mul(30).add(rootKLast.mul(38));
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        uint balance0Adjusted = (balance0.mul(10000).sub(amount0In.mul(68)));
        uint balance1Adjusted = (balance1.mul(10000).sub(amount1In.mul(68)));
